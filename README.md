...
En el archivo main.cpp, se manejaba la comunicación con la nube, de hecho, aquí es donde se realizan las llamadas a las funciones de lectura y escritura desde y hacia ThinkSpeak.
El periodo de muestreo se eligió de 5 segundos para capturar tanto las publicaciones con éxito como los errores y ver cómo se gestionan:
...
if (html_errors >= MAX_HTML_ERRORS) {
    Serial.println("Max HTML retries reached. Restarting 
    ESP32...");
    ESP.restart();
}
...
if (WiFi.status() != WL_CONNECTED) {
      ...
      while (WiFi.status() != WL_CONNECTED && wifi_attempts < 
MAX_WIFI_RETRIES) {
        WiFi.reconnect();
        Serial.print(".");
        delay(500);
